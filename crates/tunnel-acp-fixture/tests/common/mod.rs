...
//!
//! Everything here runs the gate-2 bridge directly: [`exchange`] normalizes a
//! request with `tunnel_http_bridge::forward`, carries it over a pair of
//! bounded frame queues, decodes it with `tunnel_http_bridge::serve` and hands
//! it to the ACP export's in-process handler. No listener is involved, which
//! `tests/no_listener.rs` measures on the process's own socket table.
//!
//! The second half of this file — from [`Tap`] down — is the **loopback HTTP/2
//! gateway** the pinned client talks to, and it is behind the `interop`
//! feature with the client itself. HTTP/2 is not a convenience there: the
//! pinned profile accepts HTTP/2 only (`tunnel_acp`, from the transport RFD),
//! so the gateway speaks h2 with prior knowledge and the client is built the
//! same way. Its listener is the **host** side of the conversation — the thing
//! a relay ingress would be — and the device side still has none.
//!
//! Every response body that gateway returns passes through [`Tap`], which
...
// ---------------------------------------------------------------------------
// The pinned-client gateway.
//
// Behind the `interop` feature with the client it exists for: `reqwest` is what
// carries a second `rustls` crypto provider into a build (task row M8-C09), so
// the driver and its gateway stay out of the default graph.
// ---------------------------------------------------------------------------

// `allow(unused_imports)`: this module is shared by three test binaries and
// only one of them drives the pinned client, so the re-export is unused in the
// other two even when the feature is on.
#[cfg(feature = "interop")]
#[allow(unused_imports)]
pub use gateway_support::*;
...
mod gateway_support {
    ...
    ///
    /// The listener belongs to the **test**, standing in for the relay
    /// ingress. The device side of the bridge has no listener, which is the
    /// claim `no_listener.rs` measures.
                ...
                tokio::spawn(async move {
                    let service = service_fn(move |request: Request<Incoming>| {
                        ...
                        async move {
                            ...
                            let response = super::exchange(&export, profile, request).await;
    ...
    /// A `reqwest` client that speaks HTTP/2 with prior knowledge, which is
    /// what this profile's cleartext HTTP/2-only rule requires of a consumer.
    #[must_use]
