...
impl M2Actor {
    ...
    fn refresh_pending_forget_proofs(&mut self) -> Result<(), ClientError> {
        ...
        for stream_id in stream_ids {
            ...
            if deadline.is_none_or(|deadline| now >= deadline) {
                return Err(self.expired_stream_forget_proof_error(&forget));
            }
    ...
    /// connector holds now:
    ///
    /// * evidence that **contradicts** the owner's proof (the validator fails
    ///   and the proof can no longer converge) is the peer's protocol
    ///   violation, reported as the validator's own non-retryable error;
    /// * otherwise the proof was consistent and only the relay's final
    ///   data-channel ACK had not been processed in time. That is what a
    ///   stall or a loss produces -- a laptop asleep, a process stopped, a
    ///   data path gone, or the actor's deadline tick running before the
    ///   buffered ACK after a long pause -- and the connector cannot tell it
    ///   from a relay that never sent the ACK. It is reported as a retryable
    ///   transport failure, so `connect` reconnects on a fresh session.
    ///
    ...
    fn expired_stream_forget_proof_error(
        ...
        match self.validate_stream_forget(forget) {
            Err(error) if !self.stream_forget_proof_may_converge(forget) => error,
            _ => stream_forget_proof_expired(),
        }
    }
...
mod tests {
    ...
    async fn m6c103_a_forget_proof_outlived_by_a_stall_expires_retryable() {
        ...
        // The held ACK read afterwards cannot rescue the proof; the failure
        // stays the same retryable expiry, not a protocol error.
        let error = actor
            .handle_frame(key, held_ack)
            .await
            .expect_err("a late ACK cannot rescue an expired proof");
