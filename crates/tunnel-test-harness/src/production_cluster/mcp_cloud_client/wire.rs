...
///
/// Before M3-15 the relay answered a request landing in a rotation freeze
/// with the same retryable `503 PEER_UNAVAILABLE` `not_dispatched` body as
/// every owner-not-ready fault state, so the gate sampled the owner's session
/// phase and resent only while that said a rotation was frozen.  The owner
/// now holds such a request until the freeze ends and names a freeze that
/// outlasts the hold with its own `ROTATION_FREEZE` code, so a POST no longer
/// needs this watch to decide a resend.  A standalone GET's refusal still
/// carries no body to read, so it is still resent only inside an observed
/// freeze, and a refusal outside one is still reported with the connector's
/// phase.
...
/// The message of the relay's owner-not-ready fault refusal
/// (`retryable_peer_failure_response` in `tunnel-relay/src/http.rs`).  Since
/// M3-15 no gate resends it; the tests keep it to prove that.
#[cfg(test)]
const OWNER_NOT_READY_MESSAGE: &str = "selected owner is not ready; retry after the bounded hint";

/// The relay's code for a request refused because the device's scheduled
/// data-rotation freeze outlasted the owner's bounded admission hold, or the
/// hold was full (`ROTATION_FREEZE_CODE` in `tunnel-relay/src/http.rs`;
/// M3-15).
pub(in crate::production_cluster) const ROTATION_FREEZE_CODE: &str = "ROTATION_FREEZE";

/// The retry hint of the relay's retryable **rotation-freeze** refusal in a
/// `503` body, or `None` for any other answer (M3-15).
///
/// This is the one refusal the relay itself says is the scheduled freeze, and
/// the only one a consumer should resend without its own evidence: nothing
/// was dispatched and the condition is scheduled to clear.  The
/// owner-not-ready body (`OWNER_NOT_READY_MESSAGE`) now answers only fault
/// states and is never resent, and neither is the empty-pin-set refusal
/// (M7-C83).  The hint must be present and within `1..=MIN_RETRY_HINT_MS`,
/// the relay's `OWNER_NOT_READY_RETRY_AFTER_MS` from which the freeze hint is
/// derived.
pub(in crate::production_cluster) fn rotation_freeze_hint(body: &[u8]) -> Option<Duration> {
    let value = serde_json::from_slice::<serde_json::Value>(body).ok()?;
    let hint = value["retry_after_ms"].as_u64()?;
...
/// [`rotation_freeze_hint`] for an rmcp POST failure, which carries the
/// status line and body as text.
fn not_dispatched_retry_after(
    ...
    rotation_freeze_hint(body.as_bytes())
}
...
pub struct WireCounts {
    ...
    /// POSTs the relay refused with its retryable `503 ROTATION_FREEZE`
    /// `not_dispatched` answer: a rotation freeze that outlasted the owner's
    /// bounded hold, or a full hold (M3-15).
    pub not_dispatched_refusals: u64,
    /// Those refusals sent again after the hint, at most
    /// `NOT_DISPATCHED_RETRIES` per request.  Any other refusal is never
    /// resent, so it fails its call.
    ...
    async fn post_message_with_max_sse_event_size(
        ...
        loop {
            let result = self
                ...
                .await;
            // The relay names the scheduled freeze itself (M3-15), so the
            // resend no longer waits on this gate's own freeze watch.  Any
            // other refusal, including the owner-not-ready fault body, is
            // returned to its call.
            if let Err(error) = &result
                && let Some(delay) = not_dispatched_retry_after(error)
            {
                let resend = retries < NOT_DISPATCHED_RETRIES;
                self.ledger.with(|inner| {
...
mod tests {
    ...
    #[test]
    fn only_the_rotation_freeze_503_carries_a_retry_hint() {
        ...
        let body =
            |code: &str, execution: &str, message: &str, retryable: bool, hint: Option<u64>| {
                ...
                if let Some(hint) = hint {
                    value["retry_after_ms"] = hint.into();
                }
                value.to_string()
            };
        const FREEZE_MESSAGE: &str =
            "device data rotation is in progress; retry after the bounded hint";
        let rotation_freeze = |hint: u64| {
            body(
                ROTATION_FREEZE_CODE,
                "not_dispatched",
                FREEZE_MESSAGE,
                true,
                Some(hint),
        ...
        assert_eq!(
            not_dispatched_retry_after(&refusal(&rotation_freeze(250))),
            Some(Duration::from_millis(250))
        );
        assert_eq!(
            not_dispatched_retry_after(&refusal(&rotation_freeze(1))),
            Some(Duration::from_millis(1))
        );
        // The owner-not-ready body now answers only fault states (M3-15), so
        // it is no longer resent; nor is the empty-pin-set refusal (M7-C83),
        // a freeze body with a hint above the relay's clamp or none, an
        // unknown outcome, or `retryable: false`.
        let owner_not_ready = body(
            "PEER_UNAVAILABLE",
            "not_dispatched",
            OWNER_NOT_READY_MESSAGE,
            true,
            Some(250),
        );
        let empty_pin_set = body(
            "PEER_UNAVAILABLE",
            "not_dispatched",
            "no approved peer trust evidence is published; retry after the bounded hint",
            true,
            Some(5_000),
        );
        for refused in [
            owner_not_ready.clone(),
            empty_pin_set,
            rotation_freeze(251),
            rotation_freeze(0),
            body(
            ...
            assert_eq!(
                not_dispatched_retry_after(&refusal(&refused)),
                None,
                "{refused}"
            );
        }
        assert_eq!(
            not_dispatched_retry_after(&StreamableHttpError::UnexpectedServerResponse(
                std::borrow::Cow::Owned(format!("HTTP 502 Bad Gateway: {}", rotation_freeze(250)))
            )),
            None
        );
    }
