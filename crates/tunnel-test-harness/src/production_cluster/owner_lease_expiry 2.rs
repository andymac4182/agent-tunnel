...
use super::{
    ConsumerStream, ProductionCluster, RunningHarness, STARTUP_TIMEOUT,
    assert_public_health_ready, device_dispatch_counter, start_cli_smoke,
    wait_for_fanout_drained, wait_for_public_health_ready,
};
...
/// The relay's configured owner lease.  A genuine expiry cannot be observed
/// before this much time has passed since the claim.
...
pub fn validate_owner_lease_expiry_evidence(evidence: &OwnerLeaseExpiryEvidence) -> Result<()> {
    ...
    if evidence.lease_expiry_elapsed_ms
        < u64::try_from(CONFIGURED_OWNER_LEASE.as_millis()).unwrap_or(u64::MAX)
    {
        return Err(HarnessError::Process(format!(
            "owner-lease expiry observed a disappearance after only {} ms, below the configured lease",
...
mod c17_validator_tests {
    use super::{
        HIGH_EPOCH_FLOOR, OwnerLeaseExpiryEvidence, validate_owner_lease_expiry_evidence,
    };
    ...
    fn valid_evidence() -> OwnerLeaseExpiryEvidence {
        OwnerLeaseExpiryEvidence {
            ...
            lease_expiry_elapsed_ms: 31_000,
    ...
    fn every_owner_lease_expiry_flag_and_bound_reaches_the_shared_exit_path() {
        ...
        let bounds: [Mutate; 8] = [
            ...
            ("below the configured lease", |e| {
                e.lease_expiry_elapsed_ms = 29_999
            }),
            ("bounded budget", |e| {
                e.lease_expiry_elapsed_ms = 10_000_000
            }),
...
async fn run(
    ...
    let seed = super::ownership::seed_high_owner_epoch(
        cluster,
        harness,
        device.tenant_id,
        device.id,
    ...
    .await?;
    let successor = match wait_for_owner(cluster, device.tenant_id, device.id, SUCCESSOR_TIMEOUT)
        .await
    {
    ...
    let stale_release_refused_after_successor = match observer
        .release_owner(&phase.owner_before.token)
        .await
    {
...
async fn run_partitioned_phase(
    ...
    let dispatch_before = device_dispatch_counter(&owner_relay.snapshot().await?, device_id);
    ...
    loop {
        ...
        .await
        {
            ...
            Ok(Ok(None)) => {
                ...
                owner_absent_after_lease_deadline = observed_at >= lease_deadline;
                break;
            }
    ...
    let dispatch_after = device_dispatch_counter(&owner_relay.snapshot().await?, device_id);
    let expired_owner_dispatch_unchanged = dispatch_after == dispatch_before;
    if !expired_owner_dispatch_unchanged {
        return Err(HarnessError::Process(format!(
            "owner-lease expiry advanced the owner dispatch counter from {dispatch_before} to {dispatch_after}"
        )));
    }
