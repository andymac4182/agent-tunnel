...
#    of active, preparing, quiescing, draining, committing, aborting and
#    retiring (recovering is task row M6-C136), the
...
for needle in "test result: ok. 2 passed" \
  "m606-shutdown matrix ok side=client cases=7" "m606-shutdown matrix ok side=relay cases=7" \
...
for side in client relay; do
  for phase in active preparing quiescing draining committing aborting retiring; do
