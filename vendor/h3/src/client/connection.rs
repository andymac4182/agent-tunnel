    ...
    B: Buf,
{
    ...
    /// Wait until a valid peer GOAWAY is received, without opening a request
    /// stream or waiting for the connection to become idle.
    ///
    /// The existing `wait_idle` API intentionally waits for the connection to
    /// close and therefore cannot distinguish a received GOAWAY while an
    /// admitted request is still active.  This bounded driver seam exposes
    /// exactly that control-frame transition and keeps all existing protocol
    /// error handling unchanged.  A connection close or protocol error before
    /// a GOAWAY is returned as `Err`.
    #[cfg_attr(feature = "tracing", instrument(skip_all, level = "trace"))]
    pub async fn wait_for_remote_goaway(&mut self) -> Result<(), ConnectionError> {
        future::poll_fn(|cx| self.poll_remote_goaway(cx)).await
    }

    fn poll_remote_goaway(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Result<(), ConnectionError>> {
        while let Poll::Ready(result) = self.inner.poll_control(cx) {
            match result {
                // Settings may precede a peer GOAWAY on the control stream.
                Ok(Frame::Settings(_)) => {
                    #[cfg(feature = "tracing")]
                    trace!("Got settings while waiting for peer GOAWAY");
                }
                Ok(Frame::Goaway(id)) => {
                    // A server-to-client GOAWAY carries the last
                    // client-initiated request stream ID.
        ...
        // Preserve the same client-side validation as `poll_close`: a
        // server-initiated bidirectional stream is a connection error, not a
        // reason to infer a GOAWAY.
