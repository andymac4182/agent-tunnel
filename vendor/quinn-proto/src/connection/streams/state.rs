...
impl StreamsState {
    ...
    pub(crate) fn queue_max_stream_id(&mut self, pending: &mut Retransmits) -> bool {
        ...
        for dir in Dir::iter() {
            ...
            // A fixed application stream cap can have fewer than one-eighth of its
            // window free while all previously announced stream IDs are consumed.
            // Announce every newly available slot so a single completed stream can
            // be reused without requiring a transient cap increase.
            if diff > 0 {
