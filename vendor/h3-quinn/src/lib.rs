...
impl RecvStream {
    fn new(stream: quinn::RecvStream) -> Self {
        ...
        let stream_id = num.try_into().expect("invalid stream id");
        Self {
            stream,
            stream_id,
            stopped: false,
...
impl quic::RecvStream for RecvStream {
    ...
    fn poll_data(
        ...
        let mut read = Box::pin(self.stream.read_chunk(usize::MAX, true));
        match read.as_mut().poll(cx) {
            Poll::Pending => Poll::Pending,
            Poll::Ready(Ok(chunk)) => Poll::Ready(Ok(chunk.map(|c| c.bytes))),
            Poll::Ready(Err(error)) => Poll::Ready(Err(convert_read_error_to_stream_error(error))),
        }
    }
    ...
    fn stop_sending(&mut self, error_code: u64) {
        ...
        self.stream.stop(error_code).ok();
        self.stopped = true;
    }
