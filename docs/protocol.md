...
| Owner COMPLETE | Connector RETIRED |
...
"Reserved bounded capacity" above has two separate dimensions, and the distinction was ambiguous until it was measured. Message **slots** are reserved structurally: control and data use separate bounded channels of `max_queue_messages` each, so data frames can never consume a control slot. Reserved **bytes** are not structurally separated: control and data share one `max_queue_bytes` session budget, so a workload that charges the whole budget to data would refuse every subsequent control, cancellation, revocation and rotation enqueue for want of bytes. That state is reachable by arithmetic rather than only in principle: with `max_queue_messages = 2 * max_streams_per_device` and one in-flight record per public consumer stream, `max_streams_per_device` concurrent records of 32,732 bytes each charge exactly `max_queue_bytes`, leaving zero reserved control bytes. Until a byte reservation exists, an operator profile must keep `max_streams_per_device * (2 * (max_body_bytes + 4) + frame_header)` below `max_queue_bytes` by a margin that covers the control bound, and acceptance gates must assert that margin rather than only the slot separation. See `verify-m7-queue-saturation` in [testing.md](testing.md).
