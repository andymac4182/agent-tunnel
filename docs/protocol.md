...
4. The relay sends two RESUME messages with stage `SNAPSHOT`, one per logical
   ...
   whole cumulative receive credit on the successor at activation -- the connector
   before it takes its READY snapshot, so READY never carries less credit than an
   update the relay may already have applied (M4-50, M4-52). Final reconciliation uses the fresh
