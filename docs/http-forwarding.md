...
- **Terminal discard.** After a local failure, an endpoint keeps receiving and discarding peer frames until the peer's own FIN, RESET or carrier loss, so the peer learns of the RESET in order rather than seeing a vanished receiver. The discard is bounded by the exchange deadline, so a peer that never finishes cannot hold the exchange open.
