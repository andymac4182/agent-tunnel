...
- **Handler responses.** Only `content-length` is consumed, and it must equal the body's exact size hint when both exist. For HEAD and 204/205/304 the handler may send no Content-Length field (even `0`) and no known non-zero size. The body is also drained before the head is sent, so any data is a clean pre-header `HTTP_LENGTH_MISMATCH` rather than a truncated stream. The owner generates Content-Length from a checked `body_length`, except for these zero-body responses.
- **Gateway statuses.** Before response headers commit, a failure caused by the consumer's request maps `HTTP_BODY_LIMIT` to 413, `HTTP_UNSUPPORTED_FEATURE` to 501, and any other code to 400. An upstream failure maps `HTTP_DEADLINE_EXCEEDED` to 504, `HTTP_STREAM_INTERRUPTED` with `not_dispatched` to 503, and anything else to 502. The body is exactly `{"error":{"code":"…","execution":"…"}}` with `cache-control: no-store`, and the same pair is attached as a typed response extension.
- **Execution knowledge.** The owner reports `not_dispatched` until the first REQUEST_HEAD byte is queued, then `unknown`. It reports `dispatched` once a RESPONSE_HEAD arrives, because the device only sends one after the handler returns. It adopts the device's `execution` from a device RESET. The device records `dispatched` when it starts the handler task. That decision and the first-failure record share one lock, so a RESET never claims `not_dispatched` for a handler that starts afterwards. On cancellation the device waits for the aborted handler future to be dropped before it reports.
...
- **Terminal discard.** After a local failure, an endpoint keeps receiving and discarding peer frames until the peer's own FIN, RESET or carrier loss, so the peer learns of the RESET in order rather than seeing a vanished receiver.
