...
- **Not proven by gate 3.** Rotation, freeze, recovery and progress budgets on HTTP streams (gate 4: the parking and deferred-terminal paths exist but are not exercised under a rotation). The owner does not re-validate `http-forward/1` records against a profile, although the prose above asks for owner allowlists. Cancellation reaches the device as the ordered data-socket RESET (detected out of band by the connector actor); the owner sends no control `CANCEL` for an HTTP stream, and no scoped operation-status lookup exists. `RESULT_STATUS` correlation is bounded by a timing grace, not ordered. Only HTTP/1.1 consumers are exercised. Per-profile allowlists and buffer-before-dispatch profiles remain gate 5.
