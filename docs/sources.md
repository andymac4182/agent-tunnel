...
**Not established.** No ACP client, server or agent has been run against anything in this repository: compiling against these crates is not running against them, and the profile tables passing their own tests says nothing about interoperability. crates.io's JSON API was not readable from the build host (HTTP 403), so publication timestamps come from the crates' own changelogs and from the dates of the release commits above rather than from registry metadata. No conformance suite and no non-Rust ACP SDK is pinned. The `client` and `server` features of the transport crate have never been compiled here.
