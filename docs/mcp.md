...
- **Rotation freezes refuse new requests.** The relay pauses stream admission from QUIESCE to COMMIT, so a POST can be refused with a retryable `503 PEER_UNAVAILABLE` `not_dispatched`. rmcp does not retry it; the gate's transport resends exactly that refusal and counts it (M3-15).
