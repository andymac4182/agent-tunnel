...
- **Dropped headers (M6-C58).** The public ingress removes `user-agent` and `accept-encoding` before the codec sees the request, as it removes `authorization` and `cookie` once it has verified them, unless a profile allowlists them (neither MCP profile does). Every stock HTTP client sends both by default, and refusing them failed a client's first request; dropping them changes nothing the export can observe, since the device always sends `Accept-Encoding: identity` to its backend and `identity` is acceptable to every client. They are still not forwarded.
