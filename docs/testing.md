...
The variants are `native`, `native --width 640 --height 400`, `native` with `UNAVAILABLE_WITHOUT_CONTAINER_NAME=1`, and `cua-driver`. There is no `vnc` variant, because macOS Screen Sharing is not provisioned. `--permission-reads` also records the outcome, but not the content, of `get_accessibility_tree`.
