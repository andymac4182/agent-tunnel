...
**"Every advertised OS/architecture" now has exactly one referent, and it is machine-readable.** The owner declares the set in `[workspace.metadata.release] advertised-targets` in the root `Cargo.toml`: `aarch64-apple-darwin`, `x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`. It lives in the manifest rather than in a dedicated file because a `[workspace.metadata]` table has **two independent readers** -- `tomllib` in the gate scripts, which already parse this file, and `cargo` itself via `cargo metadata --no-deps`, which surfaces the same table -- whereas a dedicated file would have only the script's own parser, so a parser fault would be invisible. `scripts/m6-release-artifact.py` **reads** that declaration and holds no copy of it; its `targets` check requires the manifest, `cargo metadata` and the set a bundle froze into its `PROVENANCE.txt` to agree, so a divergence between the declaration and what the tooling shipped goes red naming both sets. Until this declaration existed, the paragraph here said the gate's scope could not be evaluated, and it was right: a run cannot be complete against an unspecified set, and any claim that it was complete would have been a claim about a set the claimant chose. What the declaration does **not** do is build anything: one of the three triples has a passing bundle and the two Linux ones have none, because this macOS host can neither link nor execute them -- see [tasks.md](tasks.md) rows M6-01 and M6-C13, the latter recording the measured routes (Docker runs both `linux/amd64` and `linux/arm64` here; hosted CI is billing-blocked and is not a route). `.github/workflows/ci.yml`'s `os: [ubuntu-latest, macos-latest, windows-latest]` names three *runner labels* and no architecture, and is still not the answer; `deny.toml`'s reference to "the advertised release targets" now has a referent and its `[graph] targets = []` stays empty for the reason its own comment gives.
