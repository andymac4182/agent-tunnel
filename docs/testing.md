...
**Not covered yet.** Cancellation, lost-answer, unsupported-capability and permission-denied fixtures over the tunnel; the `vnc` and `cua-driver` backends through the tunnel; any macOS or Windows guest. The guest's display runs at identity scale, so it cannot answer M5-C19's non-identity question on its own. The Lane B unit and fixture tests need `cargo test -p tunnel-client --features cua`, which the workspace run does not build (M5-C24).
