...
Reachability is measured independently of the responder's own readiness. `GET /internal/v1/health` is "Authenticated version/readiness metadata, bounded response", so a relay answers an authenticated peer's bounded reachability probe whether or not its own cluster prerequisites currently hold: a route whose purpose is to report readiness cannot require the responder to already be ready. Gating that probe on the receiving relay's readiness makes each relay's route readiness depend on its peer's readiness, so a relay next to a starting, degraded, or flapping peer cannot converge even though the peer is reachable, and it makes "peer starting" indistinguishable from the blocked-UDP case which must "fail route readiness explicitly". Withdrawing readiness therefore drops reachability and capacity evidence and fences the route revision — `/readyz` and public admission fail closed at once — while keeping the verified route and pin set installed as the signed evidence of which peer certificates are approved. **Proposed, held on M7-C86, and NOT in effect.** Which unready states keep the installed pin set is a recorded owner decision whose implementation is **not** on this branch or on `main`. Today an unready runtime withdraws every approved peer pin, whatever put it in that state. The proposal narrows that: *rejected trust evidence* — an unverifiable membership authority, a record that failed verification, an expired checkpoint — would still withdraw and fail closed in both directions, which is what makes a rogue-signed record fail closed and what `m7_deployment_spki_replacement` phase 6 requires; while a *local or transient* unready state — this relay's own signed local membership missing from a candidate, a failed catalog read, a persistence failure, runtime cancellation — would retain the verified set and withdraw *readiness* instead. The motivation is that withdrawing there closes every pooled peer connection and refuses every fresh dial for as long as the condition lasts, turning one relay's transient local failure into a cluster-wide trust blackout, which a catalog read racing a membership re-sign produces routinely (M7-C81, M7-C83). **It is not implemented here because it regresses `verify-m7-trust-expiry`**; the work, its measurements and the reasoning are on M7-C86 in [tasks.md](tasks.md).
