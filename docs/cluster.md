...
| Membership record re-signed at a higher version (same node and key) | **Known interruption (defect M7-C80 in [tasks.md](tasks.md)).** Every peer admission bound to the old version is invalidated, and every in-flight peer stream with it, including a long-lived `http-forward/1` SSE response through a non-owner ingress (`HTTP_STREAM_INTERRUPTED` at ingress). New streams admit against the new record. Re-signs in quick succession previously also left peer readiness down with probes failing on a remote-closed HTTP/3 connection (defect M7-C81): a re-sign racing the periodic reconcile could leave the runtime momentarily unready, which withdrew the whole pin set and so withdrew peer trust for every peer. The **pin-withdrawal cause is fixed** — an unready runtime whose cause is local or transient keeps its verified pin set installed, as the readiness contract below already required, so a re-sign no longer refuses fresh dials to unrelated peers. The focused back-to-back re-sign regression, **M7-C83 part 1, is still open**, and M7-C81 stays planned until it lands. |
...
Reachability is measured independently of the responder's own readiness. `GET /internal/v1/health` is "Authenticated version/readiness metadata, bounded response", so a relay answers an authenticated peer's bounded reachability probe whether or not its own cluster prerequisites currently hold: a route whose purpose is to report readiness cannot require the responder to already be ready. Gating that probe on the receiving relay's readiness makes each relay's route readiness depend on its peer's readiness, so a relay next to a starting, degraded, or flapping peer cannot converge even though the peer is reachable, and it makes "peer starting" indistinguishable from the blocked-UDP case which must "fail route readiness explicitly". Withdrawing readiness therefore drops reachability and capacity evidence and fences the route revision — `/readyz` and public admission fail closed at once — while keeping the verified route and pin set installed as the signed evidence of which peer certificates are approved. Which unready states keep that set is a **recorded owner decision (2026-09-16)**, and it splits by what became unavailable.

*Rejected trust evidence withdraws the pins and fails closed.* An unverifiable membership authority, a signed record that failed verification, or an expired checkpoint leaves the relay unable to say which peer keys are approved, so it approves none, in both directions. This is what makes a rogue-signed record fail closed rather than leaving the previous key set live, and `m7_deployment_spki_replacement` phase 6 requires exactly it: under an untrusted record, relay A must refuse both the rogue and the replacement certificate on the authenticated probe.

*A local or transient unready state keeps them.* This relay's own signed local membership missing from a candidate, a catalog read that failed, a persistence failure, or runtime cancellation say nothing about whether the peer keys it already verified are still approved. Withdrawing there closed every pooled peer connection and refused every fresh dial for as long as the condition lasted, turning one relay's transient local failure into a cluster-wide trust blackout — and a catalog read racing a membership re-sign produces that condition routinely, so a consumer request landing in the window was answered as if the owner were unreachable (M7-C81, M7-C83). Those states withdraw *readiness* instead, which is what this contract already required.
...
A stronger rule was written here and reverted: that attribution should instead key on `VerifiedPeerBinding::valid_until`, the earlier of the record's and the authorising key's expiry. Instrumentation refuted the premise. The governing signed boundary is the trust/checkpoint window, which the monotonic deadline already carries and which is *earlier* than `valid_until` — measured at 11:42:26.741 against 11:43:21.399 on the same admission — so a `valid_until` arm can only fire later than the arm that already exists. See M7-C83 for the samples.

A derivation failure — a malformed digest — still fails closed to an empty snapshot in either case. Retaining never weakens admission: every forwarded stream checks its admission token against membership, an unready relay still fails `/readyz` and public admission closed, and a retained pin proves only that a certificate was approved when the set was published, because the signed record and key windows are enforced at admission (`bind_peer`/`admit_peer`) rather than by the pin set, which carries no validity of its own. A dial that does meet an empty snapshot is refused with a typed pre-dispatch outcome — nothing is written, so the consumer is told `not_dispatched` with a bounded retry hint rather than an unknown outcome it must not retry. Readiness and public admission are one decision, not two: `/readyz` and the public admission gate are the same single readiness read, so they withdraw together and recover together and no observer can see one without the other. Route readiness can still recover before a selected owner has finished re-establishing its device session; that owner answers with the documented bounded retry — `PEER_UNAVAILABLE`, `not_dispatched`, with a retry hint — which is a correct typed outcome a consumer may retry, not a readiness failure. Probe admission itself stays fail-closed: completed mTLS against the approved pin set, a relay-role certificate, and a node identifier and SPKI digest matching a currently verified route target. An unverified, wrongly pinned, wrong-role, or unapproved peer is refused, a relay with no installed route set refuses every probe, and the exchange grants one bounded payload-free stream with no owner, ticket, or dispatch state, so it is never a path around admission.
