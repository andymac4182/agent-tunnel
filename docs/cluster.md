...
| Membership record re-signed at a higher version (same node and key) | **Known interruption (defect M7-C80 in [tasks.md](tasks.md)).** Every peer admission bound to the old version is invalidated, and every in-flight peer stream with it, including a long-lived `http-forward/1` SSE response through a non-owner ingress (`HTTP_STREAM_INTERRUPTED` at ingress). New streams admit against the new record. Re-signs in quick succession have also left peer readiness down with probes failing on a remote-closed HTTP/3 connection (defect M7-C81). |
