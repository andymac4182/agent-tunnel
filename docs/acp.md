...
### Three decisions about the pinned artifacts (2026-09-26)

Each of these was left open when the pin landed. They are recorded here, next to the pin they concern. Where the owner has not decided, the recommended option is **applied by default pending owner confirmation (2026-09-25)** and can be reversed.

- **Batches (M8-C02): this profile keeps following the RFD and refuses batches.** The pinned SDK accepts batch frames. The pinned RFD revision answers them `501`. A POSTed batch is refused `501 BATCH_NOT_SUPPORTED` with nothing dispatched, and a child that writes a batch line is killed by `AcpRule::BatchNotSupported`, so the device's SSE stream never carries one. The pinned client never originates a batch; it posts one only to answer a batch it received, and it cannot receive one from this bridge. So refusing costs no interoperability that has been observed. *Applied by default pending owner confirmation (2026-09-25).* Revisit this when a real ACP peer that sends batches is met.
- **Session routing (M8-C03): the SDK's table is not normative.** `agent-client-protocol-http` 2.1.0's `method_requires_session_header` lists `session/set_model`, which `agent-client-protocol-schema` 1.7.0 does not define. `crates/tunnel-acp/src/message.rs::requires_session_header` selects this profile's session-scoped methods from the schema's own `AGENT_METHOD_NAMES`, and its doc comment records the divergence. A change that routes by the SDK's table must reconcile that entry first.
- **Key order (M8-C04): the key order of a dynamic JSON object is not part of any contract in this repository.** Pinning the SDK turns on `serde_json/preserve_order` for the whole workspace. The one observed output change is the key order of `tunnel-mcp`'s local-rejection bodies, which changed from alphabetical to insertion order. That is semantically identical JSON. Nothing signs or hashes a dynamic map. Consumers must parse JSON, not compare bytes. The feature unification is accepted with that observation on the record, rather than moving the SDK out of the shared build graph. *Applied by default pending owner confirmation (2026-09-25).* The inspection behind this covered non-test call sites. It is evidence, not proof.
...
- **What the sentinel closes, and what it does not.** Containment has two independent halves. **Trigger** — whether anything sends the group signal at all — is closed, and `crates/tunnel-acp-fixture/tests/process_residue.rs` measures it with the supervisor as a **separate process** that the test `SIGKILL`s, because a `SIGKILL` runs no `Drop`, no `kill_on_drop` and no handler and cannot be simulated from inside the process holding the handle. The in-group helper's pid is then **absent from the process table**, and it has two controls: the identical probe with the sentinel deliberately unlocatable, where the helper is required to **survive** (the pre-sentinel behaviour, so the green result cannot be a host that kills such helpers anyway), and an orderly kill of a helper of the same shape, which dies (so "containment" is not claimed for something nothing could ever have contained). **Reach** — which processes a group signal can touch — is **not** closed and the sentinel does not close it: it sends the same group signal from a different process, and `killpg`'s delivery set does not mention the sender. **A missing sentinel silently reverts to the old behaviour**, so it does not stay silent: `arm` warns once per process, `tunnel_deadman::availability()` answers before any child exists, and `tunnel-client doctor` reports `process_containment: degraded / PROCESS_CONTAINMENT_SENTINEL_MISSING` — a degradation, not a refusal, which does not change the verdict or the exit code.
- **Process-tree containment is NOT claimed, and was measured.** The fixture's `detached` mode calls `setsid` and leaves the group. After the group `SIGKILL` (one group kill recorded), `a_descendant_that_calls_setsid_survives_the_process_group_kill` reads the process table and finds the descendant **alive, in its own process group, and not a zombie** — the state check matters because `kill -0` succeeds on an unreaped corpse, so an outcome-only assertion would stay green for the wrong reason if a future containment did kill it. It survived *because* it left the group. It survived **with the sentinel armed**, which is measured rather than reasoned about, in `a_setsid_descendant_escapes_even_with_the_sentinel_armed`. That is M3-09's hole, inherited here and recorded as M8-C07 rather than assumed away. Closing it needs a kernel boundary — `cgroup v2` on Linux, a job object on Windows, a sandbox/container/VM elsewhere — and **macOS has none of them in-process**, so on macOS it is an operator constraint rather than a mechanism. Group signalling is Unix-only, and macOS is the only host any of this has run on.
...
- **No lost updates, no duplicate callbacks, no repeated side effects.** Per
  ...
  JSON-RPC id**, not by "any message carrying a stopReason" — and **no message
  is repeated under an id already seen**: the count of messages *carrying* an
  id must equal the count of distinct ids, which is the only comparison that
  detects a replay. An earlier version compared the distinct count against the
  *total* message count with `>=`, which holds by construction, had no
  falsification and no guard case, and therefore said nothing. That correlation
...
- **The documented socket count, and one new socket per round.** Every device
  socket passes a `TcpProxy`, so the count is read from underneath the tunnel:
  **two at every settled steady state** and a peak of **three**. Each rotation
  round is asserted to add **exactly one** newly seen data-socket address —
  `[1, 1, 1]` in the recorded runs — and `wait_for_rotation` is passed the
  previous active local address, so the helper's own "reused the predecessor
  data socket" check is live. The device tunnel session id and owner epoch are
  unchanged, so these are rotations and not a reconnect.

  **Both halves of that were weaker than this document said, and are fixed.**
  The rule was a cumulative `distinct_device_sockets >= 3`, which the baseline
  already half-satisfies — the control socket and the initial data socket are
  two addresses before any rotation — so a single rotation passed it. And
  `wait_for_rotation` was called with `None` for the previous address, which
  **switches off** the socket-moved check this document claimed it performed.
  The address did move in every recorded run; nothing was asserting it. That is
  exactly the "a rotation happened" versus "a counter moved" distinction the
  whole claim rests on, so it is now checked twice.
...
**And the consequence for shipped configuration, which the first draft of this
section did not draw.** `RotationConfig::default()` is **300 seconds** and a
membership record lives at most **60**. So at the default policy an ACP
connection on a non-owner ingress is invalidated *long before its first
scheduled rotation ever arrives*: the property demonstrated above is **not
reachable under default configuration**, and three rotations fit here only
because this gate runs the device at the 3-second configuration floor. The
gate's rotation interval is load-bearing for that reason, which its code
comment says and this document previously did not. What is demonstrated is that
the mechanism is sound; what is not available in production today is the
...
- **Grant revocation** withdraws the admitted exchange, and the relay's own
  ...
  **"Nothing dispatched afterwards" now has something that tried.** An earlier
  version probed only with GETs after the revocation, which start nothing, so
  `revocation_dispatched_after == 0` held whether or not the revocation worked —
  the counters could not have moved either way. The probe is now a POSTed
  `initialize`, which on this profile is exactly the request that opens a
  connection and **starts a child**, plus a prompt on the revoked principal's
  own still-open session; the attempt count is asserted before the dispatch
  delta, and both the connection and prompt counters are checked. The typed
  refusal itself — `404 SERVICE_NOT_FOUND` / `not_dispatched` — is now a
  validator rule too; it was named in this document and enforced nowhere, so a
  timed-out wait could leave whatever came last recorded with the run green.
- **Peer-path loss** and **owner loss** (the owner relay stopped) each produce
  an **explicit interruption** — the stream *fails*, rather than ending
  ...
  **The first case is no longer called peer-key rotation, and that is a
  correction.** An earlier version withdrew the ingress relay's peer pins *and*
  dropped the ingress→owner path in one step, then labelled the resulting
  interruption peer-key rotation. The interruption was the path loss: a pin set
  governs **new dials**, so an already-pooled peer connection carries an
  in-flight stream straight through a pin withdrawal. The two are now measured
...
- **The request direction of the ingress→owner peer hop was driven to its
  ...
  Recorded run: the ingress had **195,933** bytes sent toward the owner and not
  ...
  **What the earlier draft got wrong, in two ways.** It reported both figures
  ...
  direction, not the response direction. And it called the pair "saturated
  concurrently" on the strength of sampling both in one loop. That is not what
  the instrument measures: `HttpExchangeRecord` is written when an exchange
  **terminates**, so the peer-hop figure is an all-time high-water mark of
  finished exchanges, not a live gauge, and two such marks can never show two
  segments full at the same instant. The 4,000-update flood that was supposed
  to back up the response direction only produces its record after the export's
  own 30 s output-credit stall, which a 25 s sampling loop cannot see.

  So the claim is now the one the instruments support, simultaneity is listed
  in `NOT_COVERED` rather than left to be read out of a word, and the
  owner→device figure comes from `queue_bytes` and the session's own high-water
...
## The demo, and one ordering fix (2026-09-26)

**The official client through a real relay.** [demo/acp.md](demo/acp.md) runs `acp-demo-client`, a binary built from the pinned `agent-client-protocol` / `agent-client-protocol-http` `=2.1.0` client, against shipped `tunnel-relay serve` and `tunnel-client connect` processes. The relay is provisioned from `examples/m6-catalog-acp.toml`, and the device supervises `tunnel-acp-fixture agent`. One session covers `initialize`, `session/new`, a turn with five streamed updates, a permission callback answered by the client, and a turn cancelled with `session/cancel`. Every outcome is read from the message the client received. The cancellation is also read from the marker file the agent wrote. It is one relay, so the consumer enters at the owner. The non-owner path stays with `verify-m8-acp-real-path`. This is the first time the pinned client has run through a relay, not an in-process bridge. The client's own headers passed the ingress's header allowlist unchanged. The only agent is still the synthetic fixture.

**A turn's result can no longer overtake its own updates (M8-C27).** The bridge used to send a prompt's result to the session stream from the task awaiting the prompt. The agent's updates went through the connection's dispatcher. The two raced, and a consumer could see `stopReason` before an update the agent wrote earlier. That breaks "accept remaining updates until that response" above. The result is now queued on the supervisor's transport channel, behind every message the agent wrote before it, and it gets the same stall and loss rules as they do. `a_turns_result_is_never_delivered_ahead_of_its_own_updates` makes the race deterministic by parking the dispatcher behind a full stream. It is red on every run without the fix. The same holds for a turn the export answers with an error (`a_turns_error_is_never_delivered_ahead_of_its_own_updates`), and the `m8c4` guard case for the fix is attributed to both tests. The row still awaits its 50-run rate check on the three-relay gate.
