...
Release Linux relay images plus macOS ARM64, Linux x86_64/ARM64, and Windows x86_64 device binaries as platform gates pass. The relay runs only on Unix hosts: the Windows bundle carries `tunnel-client` and `tunnel-deadman`, and a Windows build of `tunnel-relay` refuses to start (task row M6-C83). Network compatibility does not imply CUA desktop support; publish adapter capability matrices separately.
