...
* **Two guards cover the window between inspecting a name and opening it**, and they were proven by deletion rather than asserted: `O_NOFOLLOW` on the open, and a comparison of the opened descriptor's identity against the inspected one. Because that window is microseconds wide, a `race-window-hook` cargo feature — off by default, in no shipped build — lets a test step into it deliberately. The directory open's `O_NOFOLLOW` alone is *not* individually load-bearing: the identity comparison catches that case too, and that is recorded rather than claimed.
...
* **Error decisions, all within the closed gate-1 vocabulary:** a special file is `ENOTSUP` (the profile does not implement it, as distinct from the host denying it); crossing a mount point, a bind mount or a macOS firmlink is `EXDEV`, decided by comparing `st_dev` with the root's; a component that changed under the resolver is `ENOENT`, because a dedicated "you lost a race" code would disclose the host's concurrent activity to a caller the grant does not entitle to it; a link target that is not valid UTF-8 is `EINVAL`, and an empty one `ENOENT`.
...
* **The export root is the only path ever opened by name**, once, at construction, from operator configuration. Moving or replacing that directory afterwards cannot redirect the export.

**Residue, not covered by gate 2 and open:** the Linux `openat2` path has never been executed; a real character or block device node could not be created without privilege, so that refusal rests on the kind decision plus the FIFO and socket cases that were demonstrated; NTFS 8.3 is closed by declaration, not by test. `Treadlink`, `Tsymlink` and `Tlink` are authorized here but not executed; directory enumeration, create, unlink, mkdir and every `Tsetattr` but size belong to gates 4 and 5; and the grant recheck after an asynchronous queue wait has no queue to wait on yet.
